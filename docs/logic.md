...
## Julian date
To easily calculate the number of days elapsed between two events, astronomers use the [Julian day](https://en.wikipedia.org/wiki/Julian_day), which is the count of days from the beginning of the Julian period. The Julian period starts at noon Universal Time on Monday, January 1, 4713 BC. 
...
Before starting the calculations to determine the moon phase, this algorithm converts the date into its corresponding Julian. The conversion works like this:
...
The illumination fraction is simply given by this formula: 

```
phaseAngle = (2π * moonAge) / fullCycleDuration
